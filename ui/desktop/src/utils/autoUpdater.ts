    ...
    if (isUsingGitHubFallback) {
      ...
      try {
        ...
        const dialogResult = (await dialog.showMessageBox({
          ...
          detail: `The update has been downloaded and extracted. To complete the installation:\n\n1. Click "Open Folder" to view the new QiaotongAgent.app\n2. Quit QiaotongAgent (this app will close)\n3. Drag the new QiaotongAgent.app to your Applications folder\n4. Replace the existing app when prompted\n\nThe update will be available the next time you launch QiaotongAgent.`,
...
export function setupAutoUpdater(tray?: Tray) {
  ...
  autoUpdater.on('update-downloaded', (info: UpdateInfo) => {
    ...
    const notification = new Notification({
      title: 'Update Ready',
      body: `Version ${info.version} will be installed when you quit QiaotongAgent. Click to install now.`,
    });
    notification.show();
...
function updateTrayIcon(hasUpdate: boolean) {
  ...
  if (hasUpdate) {
    ...
    trayRef.setToolTip('QiaotongAgent - Update Available');
  } else {
    // Use normal icon
    if (isDev) {
    ...
    trayRef.setToolTip('QiaotongAgent');
  }
