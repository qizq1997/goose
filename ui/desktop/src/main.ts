...
const MENU_TRANSLATIONS_ZH_CN: Record<string, string> = {
  ...
  'Focus QiaotongAgent Window': '聚焦 QiaotongAgent 窗口',
  'Quick Launcher': '快速启动器',
  'Always on Top': '窗口置顶',
  'Toggle Navigation': '切换导航',
  'About QiaotongAgent': '关于 QiaotongAgent',
  ...
  'Hide QiaotongAgent': '隐藏 QiaotongAgent',
  'Hide Others': '隐藏其他',
  'Show All': '全部显示',
  Services: '服务',
};
...
const createChat = async (app: App, options: CreateChatOptions = {}) => {
  ...
  if (!serverReady) {
    ...
    if (isUsingExternalBackend) {
      ...
      dialog.showMessageBoxSync({
        type: 'error',
        title: 'QiaotongAgent Failed to Start',
  ...
  // QiaotongAgent's react app uses HashRouter, so the path + search params follow a #/
  ...
  if (fileMenu?.submenu) {
    ...
    if (shortcuts.focusWindow) {
      fileMenu.submenu.append(
        new MenuItem({
          label: menuT('Focus QiaotongAgent Window'),
  ...
  if (menu && process.platform !== 'darwin') {
    ...
    if (helpMenu.submenu) {
      ...
      const aboutGooseMenuItem = new MenuItem({
        label: menuT('About QiaotongAgent'),
        submenu: Menu.buildFromTemplate([]), // Start with an empty submenu for About
      });

      // Add the Version menu item (display only) to the About app submenu
